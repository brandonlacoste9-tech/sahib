...
describe('social links', () => {
  it('exposes Instagram, Facebook, and Indeed from the live brand', () => {
    const map = Object.fromEntries(socialLinks.map((item) => [item.id, item.href]));
    expect(Object.keys(map)).toEqual(['instagram', 'facebook', 'indeed']);
    ...
    expect(map.indeed).toBe('https://ca.indeed.com/cmp/Restaurant-Sahib-Inc');
  });
});
